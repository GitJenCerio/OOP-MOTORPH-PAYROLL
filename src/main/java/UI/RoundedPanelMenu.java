...
public class RoundedPanelMenu extends JPanel {
    ...
    private static RoundedPanelMenu prevClickedButton = null;
    private JPanel targetPanel;
    private String headerText;

    public RoundedPanelMenu(Color color) {
        super();
    ...
    public void simulateClick() {
        ...
        if (targetPanel != null) {
            ...
            if (parentFrame instanceof UI_AuthorizedUserDashboard1) {
                ((UI_AuthorizedUserDashboard1) parentFrame).showPanel(targetPanel, headerText);
    ...
    private void initializeMouseListener() {
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (prevClickedButton != null && prevClickedButton != RoundedPanelMenu.this) {
                    prevClickedButton.isClicked = false;
                    prevClickedButton.repaint();
                }
                isClicked = true;
                prevClickedButton = RoundedPanelMenu.this;
                repaint();

                if (targetPanel != null) {
                    JFrame parentFrame = (JFrame) SwingUtilities.getWindowAncestor(RoundedPanelMenu.this);
                    if (parentFrame instanceof UI_AuthorizedUserDashboard1) {
                        ((UI_AuthorizedUserDashboard1) parentFrame).showPanel(targetPanel, headerText);
                    } else if (parentFrame instanceof UI_EmployeeDashboard) {
                        ((UI_EmployeeDashboard) parentFrame).showPanel(targetPanel, headerText);
